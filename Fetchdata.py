...
def clean_old_history_files(directory):
    pattern = re.compile(r'(\d{8})\.(json|meta|history\.json)$')
    latest_date = 0
    all_files = []
...
def find_and_extract_from_json(directory, keyword):
    # Construct the file path for 20240514_history.json
    file_path = os.path.join(directory, '20240514_history.json')
    
    # Check if the file exists at the specified location
    if os.path.exists(file_path):
        # If the file exists, process it to extract content based on the keyword
        return extract_section_by_keyword(file_path, keyword)
    else:
        # If the file doesn't exist, return a message indicating so
        return f"The file '20240514_history.json' does not exist in the directory: {directory}"
...
def scrape_privacy_policy(file_path, history_folder):
    ...
    print(find_and_extract_from_json(history_folder,keyword))
